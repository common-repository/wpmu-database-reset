...
class WPMU_Database_Reset {
	...
	function actions() {
		 ...
		 * Do not do this for the main website; tables may be used by other websites in the network
		 */
		if( ! is_main_site() ) {
			$prefix = $wpdb->get_blog_prefix();
			$core_tables = array_merge( $wpdb->tables( 'global' ), $wpdb->tables( 'blog' ) );
		...
		update_option( 'sidebars_widgets', array() ); // fix error in Customizer screen
		if ( ! $blog_public )
